...
export default function ForgotPasswordSheet({ visible, onClose }: Props) {
  ...
  async function handleSend() {
    ...
    await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: 'mysublist://reset-password' })
    setLoading(false)
    setSent(true)
  }
      ...
      <KeyboardAvoidingView className="flex-1 justify-end" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View className="rounded-t-3xl bg-white px-6 pt-3" style={{ paddingBottom: insets.bottom + 20 }}>
          ...
          <View className="mb-4 flex-row items-center justify-between">
            <Text className="text-[20px] font-bold text-[#111827]">Forgot Password</Text>
            <TouchableOpacity
              onPress={close}
          ...
          {sent ? (
            ...
            <>
              <Text className="mb-4 text-[14px] leading-5 text-[#6B7280]">
